...
//!
//! Note: `ExecutionPlan` is no longer represented as a callback struct here.
//! Java constructs `FFI_ExecutionPlan` directly in Java arena memory (see
//! `ExecutionPlanHandle.java`), so `scan_fn` now writes an `FFI_ExecutionPlan`
//! to an output pointer instead of returning a callbacks struct.
...
pub struct JavaTableProviderCallbacks {
    ...
    ///
    /// Java writes an FFI_ExecutionPlan struct (64 bytes) into plan_out.
    /// Returns 0 on success, -1 on error.
