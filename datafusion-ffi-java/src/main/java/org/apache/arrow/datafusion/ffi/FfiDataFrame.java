 ...
 */
public class FfiDataFrame implements AutoCloseable {
  private static final Logger logger = LoggerFactory.getLogger(FfiDataFrame.class);

  private final MemorySegment runtime;
  private final MemorySegment dataframe;
  private volatile boolean closed = false;

  FfiDataFrame(MemorySegment runtime, MemorySegment dataframe) {
   ...
   * @return A FfiRecordBatchStream for iterating over results
   * @throws RuntimeException if execution fails
   */
  public FfiRecordBatchStream executeStream(BufferAllocator allocator) {
    checkNotClosed();
    ...
    try (Arena arena = Arena.ofConfined()) {
      ...
      return new FfiRecordBatchStream(runtime, stream, allocator);
  ...
  public void close() {
    if (!closed) {
      ...
      try {
        ...
        logger.debug("Closed FfiDataFrame");
      } catch (Throwable e) {
        logger.error("Error closing FfiDataFrame", e);
      }
    }
  }
}
